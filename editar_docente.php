...
if (isset($_GET['cedula'])) {
    ...
    $consulta = "SELECT * FROM registrodocente WHERE cedula = ?";
    ...
    echo "Cedula de docente no proporcionada.";
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($cedula_actual != $cedula_nueva) {
        $verificar_cedula = "SELECT * FROM registrodocente WHERE cedula = ?";
    ...
    $actualizar = "UPDATE registrodocente SET cedula=?, nombre_docente=?, fecha_ingreso=?, edad=?, telefono=?, email=?, grado=?, fecha_nacimiento=?, sexo=?, direccion=?, observacion=? WHERE cedula=?";
    $stmt = $conexion->prepare($actualizar);
    $stmt->bind_param("sssissssssss", $cedula_nueva, $nombre_docente, $fecha_ingreso, $edad, $telefono, $email, $grado, $fecha_nacimiento, $sexo, $direccion, $observacion, $cedula_actual);

    if ($stmt->execute()) {
        echo "Los datos del docente han sido actualizados correctamente.";
        // Actualizar la cédula del estudiante en la variable $estudiante para reflejar los cambios
        $estudiante['cedula'] = $cedula_nueva;
    } else {
        echo "Error al actualizar los datos del docente: " . $conexion->error;
    }
}
...
<body>
    ...
    <div class="container">
        <a href="docenteactivos.php" class="boton-regresar">Regresar</a>
        <h2 class="title">Editar Docente</h2>
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?cedula=" . $estudiante['cedula']); ?>">
            <div class="form-container">
                <div class="form-column">
                    ...
                    <input type="text" id="nombre_docente" name="nombre_docente" value="<?php echo htmlspecialchars($estudiante['nombre_docente']); ?>">
                    <label for="fecha_ingreso">Fecha de ingreso:</label>
                    <input type="date" id="fecha_ingreso" name="fecha_ingreso" value="<?php echo htmlspecialchars($estudiante['fecha_ingreso']); ?>">
